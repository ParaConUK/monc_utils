...
.. topic:: New at 0.1
   ...
   Using :py:func:`~monc_utils.io.datain.get_data` the derivative on it's native grid (i.e. a centred difference) is returned, with the name requested. Using :py:func:`~monc_utils.io.datain.get_data_on_grid` the result is interpolated to the required grid and the name appended ``_on_grid``, with ``grid`` the requested grid.
